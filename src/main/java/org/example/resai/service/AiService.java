...
public class AiService {
    ...
    /** * Generate professional summary based on user's description * NOW WITH LANGUAGE SUPPORT! */
    ...
    /** * Generate experience bullets based on user's description * NOW WITH LANGUAGE SUPPORT! */
    ...
    /** * Generate project bullets based on user's description * NOW WITH LANGUAGE SUPPORT! */
    ...
    /** * Tailor resume data to match a specific job description * Returns the entire resume data structure with tailored content */
    ...
    private String getSystemPromptForSummary(String language) {
        if ("fr".equalsIgnoreCase(language)) {
            return """
                    Tu es un rédacteur de CV professionnel. Génère un résumé professionnel concis et optimisé pour les ATS (2-3 phrases, ~50-80 mots) basé sur la description de l'utilisateur.
                    Focus sur:
            ...
            return """
                    You are a professional resume writer. Generate a concise, ATS-friendly professional summary (2-3 sentences, ~50-80 words) based on the user's description.
                    Focus on:
    ...
    private String getSystemPromptForExperience(String role, String company, String language) {
        if ("fr".equalsIgnoreCase(language)) {
            return String.format("""
                    Tu es un rédacteur de CV professionnel. Génère 3-5 points d'expérience optimisés pour les ATS basés sur la description de l'utilisateur.
                    Contexte:
                    - Poste: %s
                    - Entreprise: %s
                    Directives:
            ...
            return String.format("""
                    You are a professional resume writer. Generate 3-5 ATS-friendly bullet points for a work experience based on the user's description.
                    Context:
                    - Role: %s
                    - Company: %s
                    Guidelines:
    ...
    private String getSystemPromptForProject(String projectTitle, String language) {
        if ("fr".equalsIgnoreCase(language)) {
            return String.format("""
                    Tu es un rédacteur de CV professionnel. Génère 3-4 points optimisés pour les ATS pour un projet basés sur la description de l'utilisateur.
                    Projet: %s
                    Directives:
            ...
            return String.format("""
                    You are a professional resume writer. Generate 3-4 ATS-friendly bullet points for a project based on the user's description.
                    Project: %s
                    Guidelines:
    ...
    private String getSystemPromptForTailoring(String language) {
        if ("fr".equalsIgnoreCase(language)) {
            return """
                    Tu es un expert en rédaction de CV et en optimisation ATS. Ta tâche est d'adapter un CV existant pour correspondre parfaitement à une description de poste spécifique.
                    Instructions:
            ...
            return """
                    You are an expert resume writer and ATS optimization specialist. Your task is to tailor an existing resume to perfectly match a specific job description.
                    Instructions:
    ...
    private String getSystemPromptForCoverLetter(String language) {
        if ("fr".equalsIgnoreCase(language)) {
            return """
                    Tu es un rédacteur professionnel de lettres de motivation. Crée une lettre de motivation convaincante et personnalisée basée sur le CV du candidat et la description de poste.
            ...
            return """
                    You are a professional cover letter writer. Create a compelling, personalized cover letter based on the candidate's resume and the job description.
                    Letter structure:
        ...
        try {
            ...
            if ("fr".equalsIgnoreCase(language)) {
                return String.format("""
                        ...
                        Adapte ce CV pour correspondre à cette description de poste. Retourne le CV modifié au format JSON avec la même structure exacte.
                        """, resumeJson, jobDescription);
            } else {
                return String.format("""
                        CURRENT RESUME (JSON): %s
                        JOB DESCRIPTION: %s
                        Tailor this resume to match this job description. Return the modified resume in JSON format with the exact same structure.
                        """, resumeJson, jobDescription);
            }
        } catch (Exception e) {
    ...
    private String callOpenAI(String systemPrompt, String userInput) {
        try {
            ...
            if (root.has("error")) {
                ...
                String errorMsg = root.path("error").path("message").asText("No details provided");
            ...
            if (choices.isMissingNode() || !choices.isArray() || choices.size() == 0) {
                throw new RuntimeException("OpenAI response missing valid 'choices' array");
            }

            JsonNode choice = choices.get(0);
            JsonNode message = choice.path("message");
            if (message.isMissingNode()) {
                throw new RuntimeException("OpenAI response missing 'message' in choice");
            }
